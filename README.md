...
docker run --rm app_api_rest php --version
docker run --rm -it app_api_rest bash

--rm : Le dice a Docker que elimine el contenedor después de que se detenga. Esto es útil para contenedores temporales que no necesitas mantener después de su uso.

composer install --no-scripts
--no-scripts : Le dice a Composer que no ejecute los scripts definidos en el archivo composer.json durante la instalación de dependencias. Esto puede ser útil si deseas instalar las dependencias sin ejecutar ningún script adicional, como migraciones de base de datos o tareas de compilación.

docker build --no-cache --progress=plain -t app_api_rest .
